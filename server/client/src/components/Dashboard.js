...
import { 
  FiServer, FiMonitor, FiTerminal, FiFolder, FiGitBranch, 
  FiUsers, FiSettings, FiLogOut, FiMenu, FiX, FiPlus,
  FiActivity, FiHardDrive, FiCpu, FiWifi
} from 'react-icons/fi';
...
function Dashboard({ user, onLogout }) {
  ...
  const navigationItems = [
    ...
    { id: 'git', label: 'Git менеджер', icon: FiGitBranch },
    ...(user.role === 'ROOT' ? [{ id: 'admin', label: 'Админ панель', icon: FiUsers }] : [])
  ];
  ...
  const fetchStats = async () => {
    try {
      ...
      const [serversRes, usersRes] = await Promise.all([
        ...
        user.role === 'ROOT' ? axios.get('/api/admin/users', { headers: { Authorization: `Bearer ${token}` } }) : Promise.resolve({ data: [] })
      ]);
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'admin':
        return user.role === 'ROOT' ? <div style={{ padding: '20px', color: '#fff' }}>
          <h2>👑 Админ панель</h2>
